...
for today = start:max_time
    ...
    % buying/ selling before major changes in the asset's value
    gold_pred = gold_pred + derivative_affect * find_derivatives(dat_gold(1:today),tolerence);
    bc_pred = bc_pred + derivative_affect * find_derivatives(dat_bc(1:today),tolerence);
    ...
    if (transaction_cost)
        % determine how much is owed in transaction cost
        total_delta = cost_gold*abs(gold-gold_old)*dat_gold(today) +...
            cost_bc * abs(bc-bc_old) * dat_bc(today);
        % remove owed money from second best (backup) asset
        if (gold_str == ind_pred_2 && cash_str ~= ind_pred_best)
    ...
    % If we cannot trade gold then undo all sales 
